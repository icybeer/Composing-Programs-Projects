...
def make_sentiment(value):
    ...
    "*** YOUR CODE HERE ***"
    def has_sentiment():
        return sentiment['value'] != None
    sentiment = {'has_sentiment':has_sentiment, 'value': value}
    return sentiment
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    none_count = 0
    sum_sentiment_value = 0
    word_list = tweet_words(tweet)
    length = len(word_list)

    for word in word_list:
        if not has_sentiment(get_word_sentiment(word)):
            none_count +=1
        else:
            sum_sentiment_value += sentiment_value(get_word_sentiment(word))
    if (none_count == length):
        return make_sentiment(None)
    else:
        return make_sentiment(sum_sentiment_value / (length - none_count))
...
def find_centroid(polygon):
    ...
    for i in range(0,length-1):
        pos_i = polygon[i]
        pos_i2 = polygon[i+1]
        common = latitude(pos_i) * longitude(pos_i2) - latitude(pos_i2) * longitude(pos_i)
        sum_area += common
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        ...
        sum_lon += lon * area
        sum_area += area
    lat_state = sum_lat / sum_area
    lon_state = sum_lon / sum_area
    return make_position(lat_state, lon_state)
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        dist_from_tweet = lambda name: geo_distance(tweet_loc, us_centers[name])
        closest_state = sorted(us_states.keys(), key=dist_from_tweet)[0]
...
def average_sentiments(tweets_by_state):
    ...
    for state, tweets in tweets_by_state.items():
        sentiment_list = [analyze_tweet_sentiment(tweet) for tweet in tweets]
        sum = 0
        count = 0
        for s in sentiment_list:
            if (has_sentiment(s)):
                sum += sentiment_value(s)
                count +=1
        if (count>0):
